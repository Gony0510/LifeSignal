...
from AI.training_presets import C4001_PRESET
from ForMov.config import DEFAULT_DATA_PATH


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "영상용 C4001 CSV에 원하는 개수만큼 증강 데이터를 추가합니다. "
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"영상용 데이터 어그멘테이션 실패: {exc}") from exc

    mode = "적용 완료" if result.applied else "미리보기"
    print(f"\n[{mode}] 영상용 C4001 CSV 어그멘테이션")
